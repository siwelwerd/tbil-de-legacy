...
\end{problem}
\begin{solution}
First, we find a general solution to the homogeneous equation \[y'+y=0.\]  This has auxilliary equation \(r+1=0\), which has a single root at \(r=1\), so \(ce^{-t}\) is a solution.  
...
%\[y = -t^2+2t-2+ce^{-t}.\]
We can use variation of parameters to find a particular solution \(y_p\); suppose \(y_p=ve^{-t}\) for some function \(v\).  Then \(y_p ' = v'e^{-t}-ve^{-t}\), so we have
\[-t^2=y_p ' + y_p = v'e^{-t}-ve^{-t} + ve^{-t} = v'e^{-t}.\]
Thus \(v'=-t^2e^t\), so (integrating by parts twice), we conclude \(v=\int -t^2 e^t\ dt  = -t^2 e^t +2te^t-2e^t\).  Thus \(y_p = ve^{-t} = -t^2+2t-2\), so the general solution is \[y=-t^2+2t-2+ce^{-t}.\]
\end{solution}
