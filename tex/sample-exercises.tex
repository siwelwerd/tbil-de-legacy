...
\begin{enumerate}[(a)]
\item Explain how to model this scenario as an initial value problem (IVP).
\item Calculate its velocity after \(0.01\ {\rm s}\).
\end{enumerate}
\end{problem}
\begin{solution}
\begin{enumerate}[(a)]
\item Let \(v\) be the velocity of the droplet.
...
where \(m=4\times 10^{-15}\operatorname{kg}\) and \(g=9.8\). To find \(b\), we note that terminal velocity 
\(v_t=-0.03\) (in meters/sec)  occurs when
